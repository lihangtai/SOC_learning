...
文件位置：/proc/devices, /dev, /sys/class, /sys/devices
...
>
>Os虚拟地址的：os加载程序到内存，内核帮我完成映射，得到虚拟地址 （两个进程运行，运行在虚拟地址上）
>
>
...
1.单片机芯片手册指明了物理地址，内存，GPIO控制器寄存器等如何进行地址排序？

属于硬件设备环节：在硬件设计完成后，根据电路和信息传输，用物理芯片自动化设计工具自动分配地址，再进行设计仿真验证。



2.对于一个飞凌 soc，板子提供驱动gpio点亮led，板子有没有电路图gpio连接led，实现按钮点亮灯 

找到板子的硬件手册，确定led的物理连接情况。编写内核态内核驱动程序，ioremap物理地址，动态挂载(.ko)/编译(.c)进内核。
