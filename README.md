...
方法1：soc + linux ：网线连接，使用静态路由：把网口的IP固定
...
内核（硬件驱动（硬件获取的事件转换为标准输入事件），输入核心层，输入事件handler（给应用层提供接口）：标准化数据传给应用程序） -》应用程序



1. 设备对象（块，字符，网络：主设备号，次设备号）  2. 设备驱动程序（设备驱动程序负责将设备对象映射到设备节点，处理中断） 3. 设备节点
...
![Screenshot 2023-05-04 at 12.52.49 PM](./image/Screenshot 2023-05-04 at 12.52.49 PM.png)

```linux

cat /proc/bus/input/devices  //获取当前系统中所有输入设备的详细信息，包括设备名称、类型、厂商、版本号、设备文件。有设备节点信息来对应

ls /dev/input/*  //获取输入设备节点有哪些
